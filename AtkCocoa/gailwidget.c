...
G_DEFINE_TYPE_WITH_CODE (GailWidget, gail_widget, AC_TYPE_ELEMENT,
                             ...
                             gpointer  data)
{
  ...
  AC_NOTE (WIDGETS, g_print ("Widget created for %s (%p) - %s\n", gtk_widget_get_name (widget), widget, G_OBJECT_TYPE_NAME (widget)));
  ...
  ac_element_set_owner (AC_ELEMENT (obj), G_OBJECT (data));
}
  ...
  if (ATK_IS_COMPONENT (accessible))
    {
      ...
      ac_element_notify (AC_ELEMENT (accessible), NSAccessibilityMovedNotification, NULL);
      ac_element_notify (AC_ELEMENT (accessible), NSAccessibilityResizedNotification, NULL);
    }
}
