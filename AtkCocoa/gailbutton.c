...
static NSArray *gail_button_real_get_actions (AcElement *element);
static gboolean gail_button_real_perform_press (AcElement *element);
...
gail_button_class_init (GailButtonClass *klass)
{
  ...
  element_class->get_actions = gail_button_real_get_actions;
  element_class->perform_press = gail_button_real_perform_press;
}
...
gail_button_get_character_count (AtkText *text)
{
  ...
  return (int)g_utf8_strlen (gtk_label_get_text (GTK_LABEL (label)), -1);
}
			           ...
			           AtkCoordType coords)
{
  ...
  label_text = gtk_label_get_text (GTK_LABEL (label));
  index = (int)(g_utf8_offset_to_pointer (label_text, offset) - label_text);
  ...
  if (index == -1)
    {
      if (coords == ATK_XY_WINDOW || coords == ATK_XY_SCREEN)
        return (int)g_utf8_strlen (label_text, -1);

      return index;  
    }
  else
    return (int)g_utf8_pointer_to_offset (label_text, label_text + index);
}
...
/* Actions */
static NSArray *
gail_button_real_get_actions (AcElement *element)
{
  return @[NSAccessibilityPressAction];
}

static gboolean
gail_button_real_perform_press (AcElement *element)
{
  GtkButton *button; 
  GtkWidget *widget;
  GailButton *gail_button;
  ...
  gail_button = GAIL_BUTTON (element);
  gail_button->action_idle_handler = 0;
  widget = GTK_ACCESSIBLE (gail_button)->widget;
