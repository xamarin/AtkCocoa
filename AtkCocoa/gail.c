...
GAIL_IMPLEMENT_FACTORY (GAIL_TYPE_NSVIEW, GailNSView, gail_nsview, GTK_TYPE_NS_VIEW)
...
GAIL_IMPLEMENT_FACTORY_WITH_FUNC_DUMMY (AC_TYPE_COMBO_CELL, AcComboCell, ac_combo_cell, GTK_TYPE_CELL_RENDERER_COMBO, ac_combo_cell_new)
...
guint ac_debug_flags = 0;
static const GDebugKey ac_debug_keys[] = {
  { "hittest", AC_DEBUG_HITTEST },
...
gail_accessibility_module_init (void)
{
  ...
  fprintf (stderr, "ATK-Cocoa started (Xcode build)\n");
  if (gail_initialized)
    {
      return;
    }
  ...
  if (a_t_support)
  */
  fprintf (stderr, "ATK-Cocoa Module initialized\n");
...
//  GAIL_WIDGET_SET_FACTORY (GTK_TYPE_CALENDAR, gail_calendar);
  ...
  GAIL_WIDGET_SET_FACTORY(GTK_TYPE_CELL_RENDERER_COMBO, ac_combo_cell);
  ...
  GAIL_WIDGET_SET_FACTORY (GTK_TYPE_NS_VIEW, gail_nsview);
